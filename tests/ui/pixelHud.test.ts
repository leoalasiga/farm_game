...
describe("pixel HUD layout", () => {
  it("keeps the stacked right-side panels inside the game canvas", () => {
    ...
    expect(controlsPanel.x).toBe(statusPanel.x);
    expect(controlsPanel.width).toBe(statusPanel.width);
    expect(controlsPanel.y).toBeGreaterThan(statusPanel.y + statusPanel.height);
